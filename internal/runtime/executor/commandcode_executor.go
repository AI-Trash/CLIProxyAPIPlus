...
func (e *CommandCodeExecutor) injectHeaders(req *http.Request, auth *cliproxyauth.Auth, stream bool) {
	...
	// Verified against command-code@1.9.0 buildCommandAuthHeaders / createApiClient.
	...
	go func() {
		...
		for scanner.Scan() {
			...
			case chunkType == "reasoning-delta":
				// command-code@1.9.0 consumeStream handles reasoning-start /
			...
			case chunkType == "finish":
				...
				if usageNode.Exists() && !usageSent {
					...
					// command-code@1.9.0 finish event: totalUsage.inputTokenDetails
				...
				// command-code@1.9.0: finishReason falls back to rawFinishReason.
...
func (e *CommandCodeExecutor) buildRequestBody(req cliproxyexecutor.Request, opts cliproxyexecutor.Options, auth *cliproxyauth.Auth) []byte {
	payload := req.Payload
	// Wire model IDs match command-code@1.9.0 catalog canonical form
	...
	if maxTokens == 0 {
		// Official CLI default: OS=64e3 in command-code@1.9.0.
		maxTokens = 64000
	}
	...
	// Params mirror command-code@1.9.0 postStream body.params: model, messages,
	...
	// Body shape mirrors command-code@1.9.0 transport.postStream({route:RS,body}):
...
// form used by command-code@1.9.0 on the /alpha/generate wire (params.model).
...
// commandCodeDeprecatedModels mirrors command-code@1.9.0 rr/or replacement maps.
...
func convertToolsForCC(translated []byte) string {
	...
	// toWireTools in command-code@1.9.0 emits {name, description, input_schema}
