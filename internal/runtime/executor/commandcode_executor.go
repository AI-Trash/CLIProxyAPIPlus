...
func (e *CommandCodeExecutor) injectHeaders(req *http.Request, auth *cliproxyauth.Auth, stream bool) {
	...
	// Verified against command-code@1.53.0 buildCommandAuthHeaders / createApiClient.
	...
	go func() {
		...
		for scanner.Scan() {
			...
			case chunkType == "reasoning-delta":
				// command-code@1.53.0 consumeStream handles reasoning-start /
			...
			case chunkType == "finish":
				...
				if usageNode.Exists() && !usageSent {
					...
					// command-code@1.53.0 finish event: totalUsage.inputTokenDetails
				...
				// command-code@1.53.0: finishReason falls back to rawFinishReason.
			...
			case chunkType == "provider-metadata":
				// New in command-code@1.51.3 (still current in 1.53.0
...
func (e *CommandCodeExecutor) buildRequestBody(req cliproxyexecutor.Request, opts cliproxyexecutor.Options, auth *cliproxyauth.Auth) []byte {
	payload := req.Payload
	// Wire model IDs match command-code@1.53.0 catalog canonical form
	...
	if maxTokens == 0 {
		// Official CLI default: ok=64e3 in command-code@1.53.0
	...
	// Params mirror command-code@1.53.0 createModelClient body.params: model,
	...
	// Body shape mirrors command-code@1.53.0 transport.postStream({route:rk,body}):
...
// matching Cr in command-code@1.53.0 dist/cli.mjs (unchanged from 1.51.3).
var ccDateSuffixRegex = regexp.MustCompile(`[-@]\d{8}$`)

// canonicalizeCommandCodeModel maps client/billing model ids to the canonical
// form used by command-code@1.53.0 on the /alpha/generate wire (params.model).
...
func canonicalizeCommandCodeModel(model string) string {
	...
	// command-code@1.53.0: strip 8-digit date suffix ([-@]\d{8}$) matching canonicalizeModelId / Cr
...
// commandCodeDeprecatedModels mirrors command-code@1.53.0 vr/kr replacement maps
...
// shape expected by command-code@1.53.0 (toWireMessages in dist/cli.mjs).
...
func convertToolsForCC(translated []byte) string {
	...
	// toWireTools in command-code@1.53.0 emits {name, description, input_schema}
